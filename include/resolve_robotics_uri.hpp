...
public:
  ...
  static fs::path resolve_robotics_uri(const std::string &uri) {
    std::vector<std::string> env_list = {
        "GAZEBO_MODEL_PATH",        "ROS_PACKAGE_PATH",
    ...
    if (scheme != "file" && scheme != "package" && scheme != "model") {
      throw std::runtime_error("URI \"" + uri +
                               "\" uses an unsupported scheme: " + scheme);
    }
    ...
    if (!model_filenames.empty()) {
      if (model_filenames.size() > 1) {
        ...
        std::cerr << ") found for uri \"" << uri
                  << "\", returning the first one." << std::endl;
      }
      return model_filenames[0];
    }

    throw std::runtime_error("No file corresponding to uri \"" + uri +
                             "\" found");
  }
};
